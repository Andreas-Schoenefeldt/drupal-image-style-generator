...
module.exports = function (options) {
    ...
    Object.keys(bpConf).forEach((bpName) => {
        ...
        if (bp.imageStyles) {
            Object.keys(bp.imageStyles).forEach((styleId) => {
                ...
                if (styleConfig) {
                    styles[bpName].styles[styleId] = {
                        aspectRatio: bp.imageStyles[styleId].aspectRatio || responsiveImageStyles[styleId].aspectRatio,
                        height: responsiveImageStyles[styleId].height
                    }
                }
            });
        }
    ...
    if (styleConfig) {
        ...
        const stylePath = styleConfig.outDir + '/_imageStyles.scss'
        const scssString = `/* ------------------------------------------------
 * ! Important: This file was generated by the npm helper drupal-image-style-generator
 * ! Important: It will be overwritten on the next run, so don't change the contents manually
 * ------------------------------------------------ 
 */
...
${Object.keys(styles).map((bpId) => {
    return `@media (${styles[bpId].mediaQuery}) {
${Object.keys(styles[bpId].styles).map(styleId => {
      const styleConf = styles[bpId].styles[styleId];
      let rule  = `  .imageStyleWrp--${styleId} {\n`;
      if (styleConf.aspectRatio) {
          rule += `    padding-bottom: ${styles[bpId].styles[styleId].aspectRatio * 100}%;\n`
      }
      if (styleConf.height) {
          rule += `    height: ${styles[bpId].styles[styleId].height}px;\n`
      }
      rule += '  }';
...
`
        fs.writeFileSync(stylePath, scssString);
